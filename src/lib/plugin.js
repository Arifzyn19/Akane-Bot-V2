...
async function loadPluginFiles(
  ...
  try {
        ...
        try {
          ...
          if (!stats.isFile()) {
            if (opts.recursiveRead)
              await loadPluginFiles(dirname, pluginFilter_, opts);
            return;
          }
          ...
          if (module && validatePlugin(module)) {
            ...
            opts.logger?.warn?.(
              `invalid plugin structure - '${formattedFilename}'`,
            ) ||
    ...
    opts.logger?.error?.(error, `error reading directory ${pluginFolder_}`) ||
      console.error(
        chalk.red(`Error reading directory ${pluginFolder_}:`, error.message),
      );
  }
...
async function reload(
  ...
  if (pluginFilter(filename)) {
    ...
    if (err) {
      logger?.error?.(
        err,
        `syntax error while loading '${formattedFilename}'`,
      ) ||
        console.error(
          chalk.red(
            `  ❌ Syntax error in ${path.basename(filename)}:`,
            err.message,
          ),
        );
    } else {
      try {
        ...
        if (module && validatePlugin(module)) {
          ...
          logger?.warn?.(`invalid plugin structure - '${formattedFilename}'`) ||
            console.log(
              chalk.yellow(
                `  ⚠ Invalid structure: ${path.basename(filename)}`,
              ),
            );
          delete plugins[formattedFilename];
        }
        ...
        logger?.error?.(e, `error require plugin '${formattedFilename}'`) ||
          console.error(
            chalk.red(
              `  ❌ Error requiring ${path.basename(filename)}:`,
              e.message,
            ),
          );
        delete plugins[formattedFilename];
